...
SANDBOX_PROVIDER=docker   # or e2b, daytona, createos, box. Keep fake only for pnpm test.
...
- **CreateOS** provides the same remote-computer contract through CreateOS desktop sandboxes. Configure `CREATEOS_SANDBOX_API_KEY` and optionally `CREATEOS_SANDBOX_BASE_URL`, `CREATEOS_SANDBOX_SHAPE`, or `CREATEOS_SANDBOX_ROOTFS`. Rakazo defaults to `https://api.sb.createos.sh`, `s-2vcpu-2gb`, and `desktop:1`.
